...
ViewTransitionsImpl = (function() {// class
    ...
    ViewTransitionsImpl.prototype.loadFirst = function(dir_, viewId_, callBack_) {
        ...
        $('#kontainer .views').remove();
        $('#header .back_button_container').addClass('first');
        self._viewsStack.splice(0, self._viewsStack.length); // reset views stack
    ...
    ViewTransitionsImpl.prototype.clearTransitionClasses = function (view) {
        view.removeClass('next-Steps').removeClass('next-new-begin').removeClass('next-new-end').removeClass('back-Steps').removeClass('back-new-begin').removeClass('back-new-end');
    };
    ...
    ViewTransitionsImpl.prototype.goBackView = function(callBack_, nViews) {
        ...
        console.log('GO BACK: views stack length :', len);
        if(len === 1) {//stack is 1
            return;
        }
        if (len === 2) {
        ...
        var i;
        for (i = 1; i < nViews; i += 1) { // some views skipping if going back more 2 or more views
            this._viewsStack.pop();
        }
